...
public class DrawingPannel extends JPanel {
    ...
    public void randomCarve() {
        if (grid == null) return;
        Random rand = new Random();
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (rand.nextDouble() > 0.6) grid[i][j].top = false;
                if (rand.nextDouble() > 0.6) grid[i][j].bottom = false;
                if (rand.nextDouble() > 0.6) grid[i][j].left = false;
                if (rand.nextDouble() > 0.6) grid[i][j].right = false;

                if (i == 0) grid[i][j].top = true;
                if (i == n - 1) grid[i][j].bottom = true;
                if (j == 0) grid[i][j].left = true;
                if (j == n - 1) grid[i][j].right = true;
            }
        }
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
    ...
    public void exportPng() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "salvat ca maze.png");
        } catch (Exception e) {}
    }
    ...
    public void saveMaze() {
        ...
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("maze.dat"))) {
            ...
            out.writeInt(n);
            JOptionPane.showMessageDialog(this, "labirint salvat in maze.dat");
        } catch (Exception e) {}
    }
